...
function start() {
    ...
    if (validateInput(x, y)) {
        ...
        let currentMovMarkImg = createCurrentMovMarkImg();
        let formatedPathArr = getKnightTourMovesSequence(x, y, chessboardSize);
        let delay = 800;

        drawKnightsTour(0, formatedPathArr, knightImg, currentMovMarkImg, undefined, delay);
    }
}
...
function drawKnightsTour(counter, formatedPathArr, knightImg, currentMovMarkImg, previousSquareTD, delay) {
    if (counter < formatedPathArr.length) {
        currentInterval = setTimeout(function () {
            ...
            if (counter === 0) {
                ...
                //Move Knight
                appendImgToSquare(firstMoveSquareTD, knightImg);
            }
            else {
                ...
                if (currItem.IsNextMove) {
                    ...
                    //currentMovMarkImg
                    appendImgToSquare(squareToVisitTD, currentMovMarkImg);

                    //Move Knight
                    appendImgToSquare(squareToVisitTD, knightImg);
                }
                else {
                    //Red Track
                    appendVisitedImgToSquare(squareToVisitTD);
            ...
            counter++;
            drawKnightsTour(counter, formatedPathArr, knightImg, currentMovMarkImg, squareToVisitTD, delay);
        }, delay);
    }
}
function appendImgToSquare(square, img) {
...
function createCurrentMovMarkImg() {
    var dv = document.createElement('div');
    dv.id = "currentMoveMarkDv";
    var knightImg = document.createElement('img');
    knightImg.id = "currentMoveMarkImg";
    knightImg.src = "image/currentMoveMark.png";
    knightImg.classList.add("currentMoveMarkImg");
    dv.appendChild(knightImg);
    return dv;
}
